...
def test_parse_appcast_without_versions():  # U39
    ...
    assert _backend.parse_appcast(empty) == {
        "latest": None,
        "latest_build": None,
        "title": None,
    }
...
def test_doctor_without_the_app_installed(runner, monkeypatch):  # U41
    monkeypatch.setattr(_backend, "backend_available", lambda: False)
    result = runner.invoke(cli, ["--json", "doctor"])
    assert result.exit_code != 0
    report = json.loads(result.output)
    assert report["verdict"] == "unavailable"
    assert report["failing"] == ["app_installed"]


def _healthy_backend(monkeypatch, helper_path=None):
    """Patch the backend into a fully-working state, app absent from the machine."""
    monkeypatch.setattr(_backend, "backend_available", lambda: True)
    # HELPER_PATH is a Path instance whose .exists is read-only; swap the
    # constant for a path that really does exist instead.
    monkeypatch.setattr(_backend, "HELPER_PATH", helper_path or pathlib.Path(__file__))
    monkeypatch.setattr(
        _backend, "app_version",
        lambda: {"version": "1.12.0", "build": "1120", "feed_url": "https://example.invalid/feed"},
    )
    monkeypatch.setattr(
        _backend, "read_prefs", lambda: {key: True for key in _backend.WRITABLE_TOGGLE_KEYS}
    )
    monkeypatch.setattr(
        _backend, "update_check",
        lambda timeout=15: {"installed": "1.12.0", "latest": "1.13.0", "up_to_date": False},
    )


def test_doctor_healthy_skips_ui_by_default(runner, monkeypatch):  # U42
    _healthy_backend(monkeypatch)
    result = runner.invoke(cli, ["--json", "doctor"])
    assert result.exit_code == 0, result.output
    report = json.loads(result.output)
    assert report["verdict"] == "healthy"
    assert report["checks"]["ui_contract"]["skipped"] is True


def test_doctor_flags_missing_defaults_keys(runner, monkeypatch):  # U43
    _healthy_backend(monkeypatch)
    monkeypatch.setattr(_backend, "read_prefs", lambda: {"trashCanSelected": True})
    result = runner.invoke(cli, ["--json", "doctor"])
    assert result.exit_code != 0
    check = json.loads(result.output)["checks"]["defaults_domain"]
    assert check["ok"] is False
    assert "userCacheFilesSelected" in check["missing"]


def test_doctor_reports_renamed_controls_without_adopting_them(runner, monkeypatch):  # U44
    """A renamed button must be reported, never learned."""
    _healthy_backend(monkeypatch)
    monkeypatch.setattr(_backend, "is_running", lambda: True)
    monkeypatch.setattr(
        _backend, "ui_snapshot",
        lambda: {"buttons": ["Clean Now", "Scan"], "texts": ["Flash Clean"], "found_junk": None},
    )
    result = runner.invoke(cli, ["--json", "doctor", "--ui"])
    assert result.exit_code == 0, "Scan is present, so the scan_action group is satisfied"
    check = json.loads(result.output)["checks"]["ui_contract"]
    assert check["found"]["Remove"] is False
    assert "Clean Now" in check["visible_buttons"]
    # The contract itself is untouched: nothing was learned from the snapshot.
    assert "Clean Now" not in _backend.UI_CONTRACT
    assert set(_backend.UI_CONTRACT) == {"Flash Clean", "Scan", "Remove"}


def test_doctor_fails_when_no_scan_control_exists(runner, monkeypatch):  # U44b
    """Both Scan and Remove gone means the cleaning flow has no anchor at all."""
    _healthy_backend(monkeypatch)
    monkeypatch.setattr(_backend, "is_running", lambda: True)
    monkeypatch.setattr(
        _backend, "ui_snapshot",
        lambda: {"buttons": ["Clean Now"], "texts": ["Flash Clean"], "found_junk": None},
    )
    result = runner.invoke(cli, ["--json", "doctor", "--ui"])
    assert result.exit_code != 0
    check = json.loads(result.output)["checks"]["ui_contract"]
    assert check["empty_groups"] == {"scan_action": ["Scan", "Remove"]}
    assert "will NOT guess" in check["detail"]
    assert set(_backend.UI_CONTRACT) == {"Flash Clean", "Scan", "Remove"}


def test_doctor_fails_when_the_pane_is_missing(runner, monkeypatch):  # U44c
    """'Flash Clean' is required always — its absence is unambiguous drift."""
    _healthy_backend(monkeypatch)
    monkeypatch.setattr(_backend, "is_running", lambda: True)
    monkeypatch.setattr(
        _backend, "ui_snapshot",
        lambda: {"buttons": ["Scan"], "texts": ["Quick Clean"], "found_junk": None},
    )
    result = runner.invoke(cli, ["--json", "doctor", "--ui"])
    assert result.exit_code != 0
    assert json.loads(result.output)["checks"]["ui_contract"]["missing"] == ["Flash Clean"]


def test_doctor_ui_requires_the_app_running(runner, monkeypatch):  # U45
    _healthy_backend(monkeypatch)
    monkeypatch.setattr(_backend, "is_running", lambda: False)
    result = runner.invoke(cli, ["--json", "doctor", "--ui"])
    assert result.exit_code != 0
    assert "not running" in json.loads(result.output)["checks"]["ui_contract"]["detail"]


def test_doctor_survives_a_broken_appcast(runner, monkeypatch):  # U46
    _healthy_backend(monkeypatch)

    def boom(timeout=15):
        raise _backend.BackendError("appcast fetch failed: timeout")

    monkeypatch.setattr(_backend, "update_check", boom)
    result = runner.invoke(cli, ["--json", "doctor"])
    assert result.exit_code != 0
    report = json.loads(result.output)
    assert report["failing"] == ["appcast"]
    assert report["checks"]["defaults_domain"]["ok"] is True, "one failure must not mask the rest"
